...
export function MobileControls() {
  ...
  useEffect(() => {
    setMobileInput({ shooting, crouching });
  }, [shooting, crouching, setMobileInput]);
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-50 flex flex-col justify-end pb-12 px-4 select-none">
      <div className="flex justify-between items-end w-full pointer-events-auto gap-2">
        ...
        {/* Center control stack */}
        <div className="flex flex-col gap-3 items-center mb-2">
          {/* Crouch Button */}
          <button
            className={`w-14 h-14 rounded-full border-2 border-cyan-400 flex flex-col items-center justify-center active:scale-95 transition-all touch-none ${crouching ? 'bg-cyan-400/50 scale-95 text-white' : 'bg-cyan-400/10 text-cyan-400'}`}
            onPointerDown={(e) => {
            ...
            style={{ touchAction: 'none' }}
          >
            <span className="text-[10px] font-bold tracking-wider uppercase">Crouch</span>
          </button>
